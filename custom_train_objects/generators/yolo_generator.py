...
class YoloGenerator(tf.keras.utils.Sequence):
    def __init__(self, images, 
        ...
        self.anchors = [BoundingBox(0, 0, config['ANCHORS'][2*i], config['ANCHORS'][2*i+1]) for i in range(int(len(config['ANCHORS'])//2))]
    ...
    def output_shapes(self):
        return (
            (self.config['IMAGE_H'], self.config['IMAGE_W'], 3), (
                (self.config['GRID_H'], self.config['GRID_W'], self.config['BOX'], 5 + len(self.config['LABELS'])),
                (1, 1, 1, self.config['TRUE_BOX_BUFFER'], 4)
            )
        )
    ...
    def load_annotation(self, i):
        ...
        for obj in self.images[i]['box']:
            annot = [obj['xmin'], obj['ymin'], obj['xmax'], obj['ymax'], self.config['LABELS'].index(obj['label'])]
            annots += [annot]
    ...
    def __getitem__(self, idx):
        if idx == 0: self.on_epoch_end()
        x_batch = np.zeros((self.config['IMAGE_H'], self.config['IMAGE_W'], 3))                         # input images
        b_batch = np.zeros((1, 1 , 1 ,  self.config['TRUE_BOX_BUFFER'], 4))   # list of self.config['TRUE_self.config['BOX']_BUFFER'] GT boxes
        y_batch = np.zeros((self.config['GRID_H'],  self.config['GRID_W'], self.config['BOX'], 5 + len(self.config['LABELS'])))               # desired network output
        ...
        for obj in all_objs:
            if obj['xmax'] > obj['xmin'] and obj['ymax'] > obj['ymin'] and obj['label'] in self.config['LABELS']:
                ...
                if grid_x < self.config['GRID_W'] and grid_y < self.config['GRID_H']:
                    obj_index  = self.config['LABELS'].index(obj['label'])
                    ...
                    y_batch[grid_y, grid_x, best_anchor, 5 + obj_index] = 1
